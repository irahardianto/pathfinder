...
fn make_unique_name(
    ...
    // PERF: Avoid HashMap::entry with .clone() to prevent unconditional allocation on cache hits
    let count = if let Some(c) = name_counts.get_mut(&name) {
        c
    } else {
        name_counts.insert(name.clone(), 0);
        name_counts.get_mut(&name).unwrap_or_else(|| unreachable!())
    };
    *count += 1;
    ...
    fn merge_recursive(syms: &mut Vec<ExtractedSymbol>) {
        ...
        syms.retain_mut(|s| {
            if s.kind == SymbolKind::Impl {
                // PERF: Avoid HashMap::entry with .clone()
                ...
                // PERF: Avoid HashMap::entry with .to_string()
...
fn walk_html_elements_flat(
    ...
    for child in node.named_children(&mut cursor) {
        ...
        if let Some(ref name) = tag_name_opt {
            ...
            // PERF: Avoid HashMap::entry with .clone()
            let count = if let Some(c) = tag_counts.get_mut(name.as_str()) {
                c
            } else {
