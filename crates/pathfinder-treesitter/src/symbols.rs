...
fn make_unique_name(
    ...
    // PERF: Avoid unconditional allocation on hash map lookup by using contains_key instead of entry(name.clone())
    if !name_counts.contains_key(&name) {
        name_counts.insert(name.clone(), 0);
    }
    #[allow(clippy::expect_used)]
    let count = name_counts.get_mut(&name).expect("inserted just above");
    ...
    fn merge_recursive(syms: &mut Vec<ExtractedSymbol>) {
        ...
        syms.retain_mut(|s| {
            if s.kind == SymbolKind::Impl {
                // PERF: Avoid unconditional allocation on hash map lookup by using contains_key instead of entry(s.name.clone())
                if !extracted_methods.contains_key(&s.name) {
                    extracted_methods.insert(s.name.clone(), Vec::new());
                }
                #[allow(clippy::expect_used)]
                let entry = extracted_methods
                    .get_mut(&s.name)
...
fn walk_html_elements_flat(
    ...
    for child in node.named_children(&mut cursor) {
        ...
        if let Some(ref name) = tag_name_opt {
            ...
            #[allow(clippy::expect_used)]
            let count = tag_counts.get_mut(name).expect("inserted just above");
    ...
    for child in node.named_children(&mut cursor) {
        match child.kind() {
            ...
            "media_statement" | "keyframes_statement" | "at_rule" => {
                ...
                // PERF: Avoid unconditional allocation on hash map lookup by using contains_key instead of entry(at_name.clone())
                if !at_counts.contains_key(&at_name) {
                    at_counts.insert(at_name.clone(), 0);
                }
                #[allow(clippy::expect_used)]
                let count = at_counts.get_mut(&at_name).expect("inserted just above");
