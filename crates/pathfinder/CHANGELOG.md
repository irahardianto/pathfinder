...
- *(explore)* add suggested_max_tokens hint and improve tool descriptions

### Fixed

- *(docs)* correct stale '3s' comments to '1s' in warmup retry paths
- *(locate)* replace blocking exists() with async tokio::fs::try_exists()
- *(read)* add safety valve to batch read concurrency throttle loop
- *(trace)* correct literal \\n in degraded-path text output
- *(search)* validate kind filter early and infer single extension from glob

### Other

- *(readme)* update language markers, file extensions, and LSP startup description
...
- *(tools)* consolidate 14 MCP tools into 7 unified entry points
...
- *(server)* map parameter validation errors to INVALID_PARAMS and clean dead imports
- update all external documentation to reflect 7-tool consolidated API
- *(docs)* synchronise internal crate doc comments with consolidated tool names
- *(types)* introduce consolidated param types and update doc references

## [0.17.1](https://github.com/irahardianto/pathfinder/compare/v0.17.0...v0.17.1) - 2026-06-13

### Fixed

- *(mcp)* generate valid input schemas for all tool definitions
- *(server)* update tool router comment metadata

### Other

- *(readme)* update tool count, architecture, and roadmap
...
- *(symbol_overview)* add granular impact_degraded and references_degraded fields

### Fixed

- *(test)* remove pre-existing auth.go before monorepo probe test
- *(find_symbol)* use treesitter SymbolKind for kind classification instead of heuristic
