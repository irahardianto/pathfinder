...
## Nit
- [x] ~~**[PAT]** Byte-based indentation logic~~ — **ALREADY RESOLVED** (already fixed in current codebase): `expand_tabs()` (indent.rs:13–29) expands tabs to 4-column boundaries *before* `min_indent` and `dedent` measure character counts, so tab-indented and space-indented input are compared and stripped consistently. `reindent` then uses all-space output. Validated by `test_dedent_tab_indented_normalises_to_spaces` (indent.rs:153–158). No action needed. — [crates/pathfinder-common/src/indent.rs](file:///home/irahardianto/works/projects/pathfinder/crates/pathfinder-common/src/indent.rs)
